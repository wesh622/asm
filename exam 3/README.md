...
Examen d'Assembleur ARM 32 bits - Part 2 - 2024-2025 / durée : 1h30
...
Dans une architecture ARM 32 bits, la taille d’un mot mémoire est :
- 1 octet
- 2 octets
- 4 octets
- 8 octets
...
Pourquoi ne peut-on pas utiliser toutes les valeurs immédiates en assembleur ARM ?
- Parce que le format des instructions limite les valeurs encodables
- Parce que certaines valeurs sont réservées à l’OS
- Parce que les instructions immédiates ne supportent que les entiers pairs
...
Un programme C compilé en ARM 32 bits sera transformé :
- en code source assembleur
- en octets ASCII
- en instructions binaires de 32 bits
- en bytecode Java
...
Parmi les instructions suivantes, laquelle permet d'appliquer un masque binaire pour tester un bit précis d'un registre ?
- AND r0, r1, #8
- ORR r0, r1, #8
- ROR r0, r1, #8
- LSL r0, r1, #8
...
En considérant les 2 erreurs comme corrigées, et en suivant le formalisme de la pile déjà vu en TD et commencée dans le fichier reponse, complétez (et modifiez si besoin) le schéma de la pile **juste avant** l'execution de la ligne 74 (label *FINAL*) du fichier ASM.
