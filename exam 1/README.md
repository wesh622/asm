...
Dans un ordinateur, le bus d'adresse est :
- un bus série
- un bus Netflix
- un bus parallèle
- un bus perpendiculaire
- un bus du réseau Stan
- un Superbus (lola, butterfly...)

Recopiez la ou les lignes vraies dans le fichier réponse.

## Question 0x02

Pourquoi dans les instructions assembleur ARM 32 bits, ne peut-on pas utiliser certaines valeurs immédiates en parametre ?
- parce que les instructions ont une taille maximale de 32 bits, paramètres compris
- Parce que certaines valeurs sont réservées à l’OS
- Parce qu’ARM utilise un codage UTF-8 pour ses instructions
...
- ajouter un nouveau disque SSD
- ajouter une nouvelle MMU
...
En considérant les 2 erreurs comme corrigées, et en suivant le formalisme de la pile déjà vu en TD et commencée dans le fichier reponse, complétez (et modifiez si besoin) le schéma de la pile **juste avant** la toute première execution de la ligne 55 (label *FINAL*) du fichier ASM.
...
Si on décommente la ligne 11, quelle est la valeur affichée à l'écran ? (Il est bien sur interdit de compiler le fichier C pour répondre à cette question).
